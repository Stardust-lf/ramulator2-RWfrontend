...
class ExposedDRAMController final : public IDRAMController,
                                    public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAMController, ExposedDRAMController,
                                    "Scheduler_expo",
                                    "Scheduler exposed controller");
...
private:
  ...
  std::string m_exetrace_path;
  std::ofstream m_exetrace_file;
  int blub_count = 0;
...
public:
  void init() override {
    ...
    m_wr_high_watermark = param<float>("wr_high_watermark")
                              ...
                              .default_val(0.8f);
    m_exetrace_path = param<std::string>("exe_trace_path")
                          .desc("Filename of the output execution trace")
                          .required();
  ...
  void setup(IFrontEnd *frontend, IMemorySystem *memory_system) override {
    ...
    m_exetrace_file.open(m_exetrace_path);
    std::cout << m_exetrace_path << std::endl;
  ...
  void tick() override {
    ...
    if (request_found) {
      std::string addr_curr = std::format("{:}", req_it->addr);
      if (prev_addr != addr_curr && blub_count != 1) {
        if (req_it->type_id == Request::Type::Read) {
          m_exetrace_file << blub_count << " R " << addr_curr << " [";
          for (size_t i = 0; i < req_it->addr_vec.size(); ++i) {
              m_exetrace_file << req_it->addr_vec[i];
              if (i < req_it->addr_vec.size() - 1) {
                  m_exetrace_file << ", ";
              }
          }
          m_exetrace_file << "]" << std::endl;
        } else {
          m_exetrace_file << blub_count << " W " << addr_curr << std::endl;
        }
        blub_count = 0;
        prev_addr = addr_curr;
      }
